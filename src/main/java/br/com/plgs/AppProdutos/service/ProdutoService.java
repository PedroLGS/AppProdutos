...
public class ProdutoService {
	...
	public Produto save(Produto produto) {
		...
		// gravar:
		try {
		return produtoRepository.save(produto); //vai para o banco de dados
		} catch(Exception e) {
	...
	public Produto update(Produto produto ) {	
		...
		if(findProduto.isPresent()) {
			...
			Produto updProduto = findProduto.get(); // set id
			...
			return produtoRepository.save(updProduto);
		} 
	 ...
	 * quantidade que seja menor que o parâmetro passado
	...
	public List<ProdutoDTO> findProdutoAndQuantidadeMenor(Integer qte) {
		...
		// converter objeto de banco de dados para ProdutoDTO
		for(Object[] obj : listResult) {
			ProdutoDTO pDTO = returnBDProdutoDTO(obj);
			listProdutoDTO.add(pDTO);
		}
	...
	private ProdutoDTO returnBDProdutoDTO(Object[] resultado) {
		...
		if(resultado != null) {
			...
			produtoDTO.setQuantidade(((Integer)resultado[4]));
		}
		return produtoDTO;
	}
	...
	private ProdutoSimplesDTO returnDBProdutoSimplesDTO(Object[] resultado) {
		if(resultado != null) {
			ProdutoSimplesDTO produtoSimplesDTO = new ProdutoSimplesDTO(
					...
					((Integer)resultado[4]).intValue()); // o preço de varejo
